...
// 20. Log if the first actor of the first movie is `Rachel McAdams` or not 
//(You have to log true or false)
...
// 21. Log if the second actor (index 1) of the second movie is `Natalie Portman` or not
// (You have to log true or false)
...
// 22. Log if the year of all three movies is greater than `2017` or not one by one.
// (Log true or false)
...
for (let i=0;i<movies.length ; i++){
  for(let j=0;j<movies[i].actors.length;j++){
    console.log(`Movie : ${movies[i].title} , Actors: ${movies[i].actors[j]}`);
  }
