...
//  Re-assign the value of `character` variable to and object with the key `characterName`
// and value of "Arya"

// - A variable named `age` is predefined with value 20. Add a new key named
// `character-age` with the value of `age` variable i.e 20 (don't use the value 20)

let age = 20;
let character ={};
character.characterName= "Arya";
character["character-age"] = 21;

// - Add a new key `sur name` with the value of the surname of the character i.e 
//"Stark". (Don't remove the space between `sur` and `name`)
...
// - Add another property named `greet` and value should be a function when called
// should alert `I am [NAME HERE] and my title is [TITLE HERE]`.
character.greet = function()
{
    alert (`I am ${character.characterName} and my title is ${character["title"]}`);
}
// - Add a new property to the character object named `isFemale` and 
//value will be `true`
character.isFemale = true;

// - Execute the function stored in the greet key of the character object
character.greet();
//  - Change the method `greet` to now alert 
//`She is [NAME HERE] and her title is [TITLE HERE]`.
...
character.greet();
//  - Add a new property in the character object `isAdult`. 
//The value will be true if age is greater than 18 or it will be false.
...
//When called should be able to flip the value of `isAdult` from true to false and 
//false to true.
...
// - Add a new key with the name stored in variable `keyName` defined
// below i.e `playedBy` (use [] for computed property) and the value of "Maisie Williams"

let keyName = 'playedBy';
character["playedBy"] ="Massie Williams";
...
// - Using the function `console.log` log the value of the key `42`. You don't have 
//to add the key
console.log(`${character["42"]}`);
// - Add a new key named `totalSeasons` with the value of `1 + 2 + 3 + 2`.
// Use this expression
