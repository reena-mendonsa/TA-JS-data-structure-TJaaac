...
Try to first understand how Array in Objects & Objects in Array are arranged in this
 example.
...
// 7. Log the name and description of all the people in house Lannisters (index 1) 
//one by one like `I am [name] and my bio is [description]`
for(let i=0;i<got.houses[1].people.length;i++){
  console.log(`I am ${got.houses[1].people[i].name} and my bio is ${got.houses[1].people[i].description}`);
}
// 8. Log the name and description of all the people in house Starks (index 0) 
//one by one like `I am [name] and my bio is [description]`
for(let i=0;i<got.houses[0].people.length;i++){
  console.log(`I am ${got.houses[0].people[i].name} and my bio is ${got.houses[0].people[i].description}`);
}
// 9. Log the name and description of the people in house Starks whose 
//name is `Robb Stark`
for(let i=0;i<got.houses[0].people.length;i++){
...
// 10. Log the name and description of the people in house Lannisters whose name 
//is `Tywin Lannister`
for(let i=0;i<got.houses[1].people.length;i++){
...
// 14. Add the name and size of people in an object like {Starks: 4, Lannisters: 6} 
//and log the object
let obj={};
obj[got.houses[0].name] = got.houses[0].people.length;
obj[got.houses[1].name] = got.houses[1].people.length;
console.log(obj);
// 15. Log the name of all the people of all the houses in got one by one
for (let i=0; i<got.houses.length;i++){
  for(let j=0;j<got.houses[i].people.length;j++){
    console.log(`From the house of ${got.houses[i].name} ::`);
...
// 20. Only the the description whose length is more than 30
//. (use .length property on string to get the length)
for(let i=0 ; i<allDescription.length;i++){
